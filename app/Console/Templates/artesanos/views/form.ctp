				...
				switch($field){
					...
					default:
						if (substr($field,-3) !="_id" && $field !="id"){
							echo "\t\techo \$this->Form->input('{$field}',array('label'=>'".ucfirst(Inflector::humanize(substr($field,4,strlen($field))))."'));\n";
						}else{
							echo "\t\techo \$this->Form->input('{$field}');\n";
						}
					break;
		...
		if (!empty($associations['hasAndBelongsToMany'])) {
			foreach ($associations['hasAndBelongsToMany'] as $assocName => $assocData) {
				if (substr($field,-3) !="_id"){
					echo "\t\techo \$this->Form->input('{$assocName}',array('label'=>'".ucfirst(Inflector::humanize(substr($field,4,strlen($field))))."'));\n";
				}else{
