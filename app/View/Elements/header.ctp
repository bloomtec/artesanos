<div id="header">
	...
	<ul id='user-menu'>
		...
		<?php }else{?>
			<li><?php echo 'Ha iniciado sesión como <b>' . $this -> Session -> read("Auth.User.username") . '</b> :: ' . $this -> Html -> link(__('Cerrar Sesión',true),array('controller' => 'usuarios', 'action' => 'logout')); ?></li>
		<?php } ?>
	</ul>
</div>